...
class PongConsumer(AsyncWebsocketConsumer):
	...
	async def update_ball(self):
		...
		while True:
			...
			if ball['x'] <= 15 + ball_radius and PongConsumer.paddles[self.room_id]['left'] - paddle_height // 2 <= ball['y'] <= PongConsumer.paddles[self.room_id]['left'] + paddle_height // 2:
				if ball['x'] <= 14 + ball_radius:
					ball['x'] = 15 + ball_radius
				direction['x'] *= -1
				direction['x'] *= acceleration
			
			if ball['x'] >= 885 - ball_radius and PongConsumer.paddles[self.room_id]['right'] - paddle_height // 2 <= ball['y'] <= PongConsumer.paddles[self.room_id]['right'] + paddle_height // 2:
				if ball['x'] >= 886 - ball_radius:
					ball['x'] = 885 - ball_radius
				direction['x'] *= -1
				direction['x'] *= acceleration
