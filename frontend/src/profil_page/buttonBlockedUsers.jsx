    ...
    <Dropdown className="custom-dropdown-blocked">
      ...
      <Dropdown.Menu>
		<Dropdown.Item>
			...
			{blockedUsers.length !== 0 && (
				<div className="test">
					{blockedUsers.map((userBlocked, index) => (
						<div className="blockedUser-item">
							<div key={index} className="blockedUser-item-username">
								<span className="modifyUsername">{userBlocked.username}</span>
							</div>
							<div key={index} className="blockedUser-item-cross" onClick={() => handleUnblock(userBlocked)}>
								<i class="bi bi-x-lg modifyCross"></i>
							</div>
						</div>
					))}
				</div>
